...
/**
 * useEducationManager Hook
 * Manages education entries with validation
 */

import { useState, useCallback } from "react";
import type { EducationEntry, ProfileValidationError } from "../types";
import { validateEducationEntry } from "../helpers/validation";
...
interface UseEducationManagerReturn {
    entries: EducationEntry[];
    errors: Map<string, ProfileValidationError[]>;
    addEntry: (entry: Omit<EducationEntry, "id">) => void;
    updateEntry: (id: string, updates: Partial<EducationEntry>) => void;
...
/**
 * Generate unique ID for education entry
 */
...
/**
 * Hook to manage education entries
 */
export function useEducationManager({
    initialEntries = [],
    onChange,
}: UseEducationManagerProps = {}): UseEducationManagerReturn {
    ...
    const [entries, setEntries] = useState<EducationEntry[]>(entriesWithIds);
    ...
    const addEntry = useCallback(
        (entry: Omit<EducationEntry, "id">) => {
            ...
            const validationErrors = validateEducationEntry(newEntry);
    ...
    const updateEntry = useCallback(
        (id: string, updates: Partial<EducationEntry>) => {
            ...
            if (updatedEntry) {
                const validationErrors = validateEducationEntry(updatedEntry);
                setErrors((prev) => {
    ...
    const validateEntry = useCallback(
        (id: string): boolean => {
            ...
            const validationErrors = validateEducationEntry(entry);
            setErrors((prev) => {
                const newErrors = new Map(prev);
                if (validationErrors.length > 0) {
                    newErrors.set(id, validationErrors);
