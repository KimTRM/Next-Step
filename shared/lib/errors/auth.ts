...
export function handleAuthError(
  ...
  if (options?.showToast !== false) {
    if (appError.code === 'UNAUTHORIZED' || appError.code === 'TOKEN_EXPIRED') {
      showAuthErrorToast(appError, options?.onSignIn);
    } else if (isRetryableError(appError)) {
      showRetryToast(appError, options?.onRetry || (() => window.location.reload()));
    } else {
...
export function handleClerkError(
  ...
  // Clerk error mapping
  const clerkErrorMap: Record<string, { code: AuthErrorType; message: string }> = {
  ...
  const errorInfo = clerkErrorMap[error?.errors?.[0]?.code] || {
...
export function useAuthErrorHandler() {
  ...
  const handleClerkError = (error: any, context?: AuthContext) => {
  ...
  const checkSession = (session: any, context?: AuthContext) => {
  ...
  const handleAuthz = (
    ...
    return handleAuthzError(resource, action, userRole, requiredRole, context);
  };
 ...
 */
export function withAuthHandling<T extends any[], R>(
  fn: (...args: T) => Promise<R>,
  context?: AuthContext
) {
