...
class ToastManager {
    ...
    error(
        ...
        const description =
            error instanceof NextStepError ? error.userMessage : error?.message;

        toast.error(message, {
            duration:
                options?.duration ||
                ((
                    error instanceof NextStepError &&
    ...
    update(
        id: string,
        options: { message: string; type?: ToastType; description?: string },
    ): void {
        try {
            ...
            return {
                ...
                onClick: () => {
                    ...
                    window.dispatchEvent(
                        new CustomEvent("retry-error", { detail: error }),
                    );
                },
            };
        }
    ...
    authError(navigate?: (path: string) => void): void {
        this.error("Authentication required", undefined, {
    ...
    permissionError(): void {
        this.error("Access denied", undefined, {
            description: "You don't have permission to perform this action.",
        });
    }

    serverError(): void {
        this.error("Server error", undefined, {
            description:
