    ...
    showError('Authentication required', nextStepError, {
      action: {
        ...
        onClick: () => {
          ...
          if (currentPath !== signInPath && currentPath !== signUpPath) {
...
export function requirePermission(
  ...
  if (!hasPermission) {
    const { showMessage = true, fallbackPath = '/' } = options || {};

    if (showMessage) {
      showWarning(`You need ${permission} permission to access this feature`, {
        action: fallbackPath ? {
          label: 'Go Back',
          onClick: () => {
